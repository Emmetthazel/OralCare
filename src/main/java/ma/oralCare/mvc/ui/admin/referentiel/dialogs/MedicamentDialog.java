...
/**
 * Dialogue Premium pour la gestion des médicaments
 * Design moderne avec espacements généreux et composants élégants
 */
public class MedicamentDialog extends BaseReferenceDialog {
    
    private JTextField txtNom, txtLabo, txtType, txtPrix;
    private JComboBox<FormeMedicament> cboForme;
    private JCheckBox chkRemb;
    ...
    public MedicamentDialog(Component parent, SystemReferentielController controller, Medicament medicament) {
        super(parent, controller, medicament == null || medicament.getIdEntite() == null ? 
              "Nouveau Médicament" : "Modifier Médicament");
    ...
    private void loadData() {
        if (medicament.getIdEntite() != null) {
            txtNom.setText(medicament.getNom() != null ? medicament.getNom() : "");
            txtLabo.setText(medicament.getLaboratoire() != null ? medicament.getLaboratoire() : "");
            txtType.setText(medicament.getType() != null ? medicament.getType() : "");
            cboForme.setSelectedItem(medicament.getForme() != null ? medicament.getForme() : FormeMedicament.TABLET);
            txtPrix.setText(medicament.getPrixUnitaire() != null ? medicament.getPrixUnitaire().toString() : "");
            chkRemb.setSelected(medicament.getRemboursable() != null && medicament.getRemboursable());
        }
    }
    ...
    protected JPanel createFormPanel() {
        ...
        // Nom Commercial
        panel.add(createPremiumLabel("💊 Nom Commercial"), gbc);
        gbc.gridy = ++row;
        txtNom = createPremiumTextField(25);
        panel.add(txtNom, gbc);
        
        // Laboratoire
        gbc.gridy = ++row;
        panel.add(createPremiumLabel("🏭 Laboratoire"), gbc);
        gbc.gridy = ++row;
        txtLabo = createPremiumTextField(25);
        panel.add(txtLabo, gbc);
        
        // Type/Molécule
        gbc.gridy = ++row;
        panel.add(createPremiumLabel("🧪 Type/Molécule"), gbc);
        gbc.gridy = ++row;
        txtType = createPremiumTextField(25);
        panel.add(txtType, gbc);
        
        // Forme Pharmaceutique
        gbc.gridy = ++row;
        panel.add(createPremiumLabel("💊 Forme"), gbc);
        gbc.gridy = ++row;
        cboForme = new JComboBox<>(FormeMedicament.values());
        styleElegantComboBox(cboForme);
        panel.add(cboForme, gbc);
        
        // Prix Unitaire
        gbc.gridy = ++row;
        panel.add(createPremiumLabel("� Prix Unitaire (DH)"), gbc);
        gbc.gridy = ++row;
        ...
        // Remboursable
        gbc.gridy = ++row;
        chkRemb = createElegantCheckBox("✅ Médicament remboursable");
        panel.add(chkRemb, gbc);
    ...
    private JPanel createPremiumFooter() {
        ...
        btnSave.addActionListener(e -> {
            if (validateAndSave()) {
                showElegantSuccess("Médicament enregistré avec succès");
                dispose();
            }
        });
    ...
    protected boolean validateAndSave() {
        if (txtNom.getText().trim().isEmpty()) {
            showElegantError("Le nom du médicament est obligatoire");
            return false;
        }
        
        try {
            medicament.setNom(txtNom.getText().trim());
            medicament.setLaboratoire(txtLabo.getText().trim());
            medicament.setType(txtType.getText().trim());
            medicament.setForme((FormeMedicament) cboForme.getSelectedItem());
            medicament.setRemboursable(chkRemb.isSelected());
            
            String prixText = txtPrix.getText().trim().replace(",", ".");
            medicament.setPrixUnitaire(prixText.isEmpty() ? BigDecimal.ZERO : new BigDecimal(prixText));
            
            controller.updateMedicament(medicament);
            return true;
            
        } catch (NumberFormatException e) {
            showElegantError("Le prix doit être un nombre valide");
            return false;
        } catch (Exception e) {
