/*package ma.oralCare.service.modules.agenda.impl;
...
public class AgendaServiceImpl implements AgendaService {
    ...
    public Optional<AgendaMensuel> getAgendaForMedecinAndMonth(Long medecinId, int annee, int mois) {
        ...
        Mois m = Mois.values()[mois - 1]; // mois 1-12
    ...
    public AgendaMensuel saveAgenda(AgendaMensuel agenda) {
        ...
        if (agenda.getIdEntite() == null) {
            ...
            agendaRepository.setJoursNonDisponible(agenda.getIdEntite(), agenda.getJoursNonDisponible());
            agenda = agendaRepository.findById(agenda.getIdEntite()).orElse(agenda);
        }
    ...
    @Override
    public List<RDVDisplayModel> getTodayAppointments() {
        return rdvRepository.findByDate(LocalDate.now())
                .stream()
                .map(rdv -> new RDVDisplayModel(
                        // On convertit l'heure (LocalTime) en String "HH:mm"
                        rdv.getHeure().toString(),

                        // Nom du patient (String)
                        rdv.getDossierMedicale().getPatient().getNom(),

                        // Prénom du patient (String)
                        rdv.getDossierMedicale().getPatient().getPrenom(),

                        // Motif / Type de soin (String)
                        rdv.getMotif(),

                        // Statut (String) - On récupère le nom de l'énumération
                        rdv.getStatut() != null ? rdv.getStatut().name() : "PRÉVU"
                ))
